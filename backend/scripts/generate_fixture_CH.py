...
import csv
plzs = {}
with open("../apps/mapview/files/CH.csv", encoding='utf-8') as csvfile:
  reader = csv.DictReader(csvfile, fieldnames=["plz","lat","lon","ort"])
  next(csvfile)
  for row in reader:
      plzs[row["plz"]] = (float(row["lon"]), float(row["lat"]), row["ort"])

# Faker Paket hat leider keine schweizer Lokalisierung
german_fake = Faker(['de_DE'])
# swiss_fake = Faker(['ch_DE'])
...
for i in range(1000):
    fixture += generate_student(i+1001, random_plz)

for i in range(20):
    fixture += generate_hospital(i+2001, big_city_plz)

for i in range(10):
    fixture += generate_hospital(i+2021, random_plz)

fixture += fixture_end

with open("../fixture_CH.json", "w") as f:
    f.write(fixture)